...
/**
  * This is an event sourced entity. It has a state, [[BottyState]], which
  ...
  */
class BottyEntity extends PersistentEntity {

  override type Command = BottyCommand[_]
  override type Event = BottyEvent
  override type State = BottyState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: BottyState = BottyState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case BottyState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        BottyState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class BottyState(message: String, timestamp: String)
...
object BottyState {
  /**
    ...
    */
  implicit val format: Format[BottyState] = Json.format
}

/**
  * This interface defines all the events that the BottyEntity supports.
  */
sealed trait BottyEvent extends AggregateEvent[BottyEvent] {
  def aggregateTag = BottyEvent.Tag
}

object BottyEvent {
  val Tag = AggregateEventTag[BottyEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends BottyEvent
...
/**
  ...
  */
sealed trait BottyCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends BottyCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String) extends BottyCommand[String]
...
/**
  ...
  * application loader.
  */
object BottySerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    JsonSerializer[UseGreetingMessage],
    JsonSerializer[Hello],
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[BottyState]
  )
}
